...
export function getGitHubUrl(repo: GitRepo): string | undefined {
  // 1. Check git remotes for a GitHub remote
  const githubRemote = repo.remotes.find((r) => r.host === "github.com");
  if (githubRemote) {
    return githubRemote.url;
  }
  // 2. Fallback: extract from filesystem path (e.g. ghq: .../github.com/owner/repo)
  const match = repo.fullPath.match(/github\.com\/(.+)$/);
  if (match) {
